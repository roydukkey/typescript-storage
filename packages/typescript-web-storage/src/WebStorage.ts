...
export abstract class WebStorage implements TypedStorage {
	...
	addListener<T extends StoreValue, R> (key: string, listener: { (event: WebStorageEvent<T>): R }): typeof listener {
	 ...
	 * @returns `true` when successfully removed at least one listener; otherwise, `false`.
	 */
	removeListener<T extends StoreValue, R> (key: string, listener?: { (event: WebStorageEvent<T>): R }): boolean;
